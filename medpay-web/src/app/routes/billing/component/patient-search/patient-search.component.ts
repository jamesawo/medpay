...
export class PatientSearchComponent implements OnInit, OnDestroy {
    ...
    public options: PatientPayload[] = [];

    public isLoading = false;
    public searchInput$ = new Subject<string>();
    public minLengthTerm = 4;

    private url: string = environment.api.baseUrl + '/billing/patient/search';
    private loading: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(false);
    private subscription: Subscription = new Subscription();

    @Output('selected')
    public selected: EventEmitter<PatientPayload> = new EventEmitter<PatientPayload>();
    ...
    ngOnInit(): void {
        if (this.selectedPatient) {
            if (this.selectedPatient.id) {
                this.options.push(this.selectedPatient);
            } else {
    ...
    public onSelected(value: PatientPayload) {
        if (value) {
    ...
    contactLabel(data: PatientPayload) {
        return `${data.uniqueCode} - ${data.firstName} ${data.lastName} ${data.otherName}`;
    }
}
