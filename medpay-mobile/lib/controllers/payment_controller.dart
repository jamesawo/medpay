...
class PaymentController extends GetxController {
  ...
  Future<void> onSearchBillDetails(String billNumber) async {
    ...
    try {
      _searchResult = [];
      isSearching = true;
      String? hospitalId = Get.find<AppSharedPreference>().getValue(AppConstants.keyHospitalId);
      Response response = await paymentRepository.doSearchBillDetails(billNumber, hospitalId!);
      if (response.statusCode == 200) {
        ...
        isSearching = false;
        HelperUtils.showErrorAndLog(response, '${(this).toString()} ${(this).onSearchBillDetails.toString()}');
        return;
      }
    } catch (e) {
  ...
  Future<void> handleBillPayment(HospitalBill bill, BuildContext context) async {
    try {
      isMakingPayment = true;
      TransactionModel transaction = _prepTransactionForBillPayment(bill);
      Response response = await paymentRepository.doTransactionPayment(transaction);
      if (response.statusCode == 200) {
        Map<String, dynamic> body = jsonDecode(jsonEncode(response.body));
        ...
        isMakingPayment = false;
        HelperUtils.showErrorAndLog(response, '${(this).toString()} ${(this).handleBillPayment.toString()}');
        return;
      }
    } catch (e) {
      isMakingPayment = false;
      ...
      LoggerUtils.logError(error: e, caller: '${this}.toString() $handleBillPayment.toString()');
      return;
    }
  }
  ...
  TransactionModel _prepTransactionForBillPayment(HospitalBill bill) {
    ...
    HospitalModel selectedHospital = Get.find<HospitalController>().selectedHospital;
    ...
    TransactionPaymentDetailModel paymentDetailModel = TransactionPaymentDetailModel(billNumber: bill.billNumber);
  ...
  ///
  /// used when hospital hasHospitalSoftware is false
  Future<void> handleServicePayment({required String name, required String phone, required String amount}) async {
    try {
      isMakingPayment = true;
      TransactionModel transactionModel = _prepTransactionForServicePayment(name, phone, amount);
      Response response = await paymentRepository.doTransactionPayment(transactionModel);
      if (response.statusCode == 200) {
        Map<String, dynamic> body = jsonDecode(jsonEncode(response.body));
        TransactionModel transactionResponse = TransactionModel.fromJson(body);
        isSearching = false;
  ...
  TransactionModel _prepTransactionForServicePayment(String name, String phone, String amount) {
    UserModel loggedInUser = Get.find<AuthController>().loggedInUser;
    ...
    TransactionPayerDetailModel payerDetail = TransactionPayerDetailModel(fullName: name, phoneNumber: phone);
    TransactionPaymentDetailModel paymentDetailModel = TransactionPaymentDetailModel(services: hospitalController.services);
    return TransactionModel(
  ...
  void _onNavigateToPaymentStatusScreen(TransactionModel data) {
    ...
    HelperUtils.popBeforeNavigateToWithArgs(AppRoutes.paymentStatusScreen, {'transaction': data});
  }
