
package com.james.medpay.features.hospital.domain.usecase.apiUsage;

import com.james.medpay.core.annotation.UseCase;
import com.james.medpay.core.contract.UsecaseContract;
import com.james.medpay.core.params.Param;
import com.james.medpay.features.hospital.domain.entity.partial.HospitalBill;
import com.james.medpay.features.hospital.domain.repository.contract.IHospitalApiUsageRepository;
import lombok.RequiredArgsConstructor;

import java.util.Optional;

@UseCase
@RequiredArgsConstructor
public class HospitalApiUsageSearchBillUsecase implements UsecaseContract<Optional<HospitalBill>, Param<String>> {

	private final IHospitalApiUsageRepository apiUsageRepository;

	@Override
	public Optional<HospitalBill> execute( Param<String> param ) {
		return apiUsageRepository.apiSearchForBillDetail( param.getId(), param.getModel() );
	}
}
