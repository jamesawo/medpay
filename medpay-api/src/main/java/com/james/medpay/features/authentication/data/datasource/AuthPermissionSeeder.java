...
public class AuthPermissionSeeder implements CommandLineRunner {
	...
	public static final String SUPER_ADMIN_PASSWORD = "password@1";
	...
	private void seedDefaultSystemUser() {
		...
		if (optional.isEmpty()) {
			...
			user.setEmail(SUPER_ADMIN_USERNAME);
			user.setExpiryDate(LocalDate.now().plusMonths(40));
	...
	private void setUserBasicDetails(UserEntity user) {
		...
		// details.setEmailAddress( SUPER_ADMIN_EMAIL );
		details.setFirstName("SUPER");
