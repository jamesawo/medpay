...
public class UserUpdateEmergencyAddressUsecase implements UsecaseContract<Optional<IUserAddressEntity>, Param<UserAddressEntity>> {

	private final IUserAddressRepository repository;

	@Override
	public Optional<IUserAddressEntity> execute(
			Param<UserAddressEntity> param
	) {
		return repository.updateEmergencyAddressByUserId( param.getId(), param.getModel() );
	}
}
